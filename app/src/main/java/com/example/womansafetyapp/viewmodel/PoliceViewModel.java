    ...
    public void observeAlerts() {
        ...
        listener = emergencyRepository.observeAlerts(new EmergencyRepository.AlertsListCallback() {
            ...
            public void onSuccess(List<EmergencyAlert> alerts) {
                List<EmergencyAlert> visible = new ArrayList<>();
                for (EmergencyAlert alert : alerts) {
                    if (alert.getTypeEnum() != null && alert.getTypeEnum().isVisibleToPolice()) {
                        visible.add(alert);
                    }
                }
                visible.sort((a, b) -> Long.compare(b.getTimestamp(), a.getTimestamp()));
                alertsState.setValue(Resource.success(visible));
            }
